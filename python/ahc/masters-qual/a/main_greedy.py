...
class State:
    def __init__(self, N, w_walls, h_walls, values) -> None:
        ...
        self.p1_visited = [[False] * self.N for _ in range(self.N)]
        self.p2_visited = [[False] * self.N for _ in range(self.N)]
    ...
    def next_action(self):
        n_p1s = self._move_to(self.p1[0], self.p1[1], self.p1_visited)
        n_p2s = self._move_to(self.p2[0], self.p2[1], self.p2_visited)
        if not n_p1s and not n_p2s:
            self.reset_visit()
            n_p1s = self._move_to(self.p1[0], self.p1[1])
            n_p2s = self._move_to(self.p2[0], self.p2[1])
    ...
    def update(self, p1, p2):
        ...
        self.p1_visited[self.p1[0]][self.p1[1]]
        self.p2_visited[self.p2[0]][self.p2[1]]
