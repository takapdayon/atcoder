...
class State:
    ...
    def do(self):
        ...
        for t in range(self.k):
            ...
            for h in range(3):
                for w in range(3):
                    self.arows[h + rh][w + rw] = (self.arows[h + rh][w + rw] + self.stamp[rs][h][w]) % MOD
            self.ans[t] = (rs, rh, rw)
...
def main():
    ...
    time_keeper = TimeKeeper(1770)
    ans = []
    state = State(arows, stamp, K, N)
    ans_state = state
    ...
    while True:
        ...
        new_state.do()
        if ans_state.score() < new_state.score():
            ans_state = new_state
            ans = new_state.get_ans()
